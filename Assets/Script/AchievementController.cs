...
public class AchievementController : MonoBehaviour
{
    ...
    [SerializeField]
    private List<AchievementData> _achiecementList;
    ...
    private void Update()
    {
        if (_popUpShowDurationCounter > 0)
        {
            ...
            _popUpTransform.localScale = Vector3.LerpUnclamped
                (_popUpTransform.localScale, Vector3.one, 0.5f);
        }
        else
        {
            _popUpTransform.localScale = Vector2.LerpUnclamped
                (_popUpTransform.localScale, Vector3.right, 0.5f);
        }
    }
    public void UnlockAchievement(AchievementType type, string value)
    {
        AchievementData achievement = _achiecementList.Find(a =>  a.Type == type && a.Value == value);
